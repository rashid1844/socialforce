...
Changes:
- support GPU + JIT
- spport unique params for Robot
- add analytic gradient for standard SFM (makes it faster)
...
from .field_of_view_robot import FieldOfView
...
class Simulator(torch.nn.Module):
    ...
    def normalize_state(self, state):
        if not isinstance(state, torch.Tensor):
            state = torch.tensor(state, dtype=self.dtype, device=state.device)
        ...
        if state.shape[1] in (6, 7):
            ...
            no_accelerations = torch.zeros((state.shape[0], 2), dtype=state.dtype, device=state.device)
        ...
        if state.shape[1] == 8:
            ...
            if hasattr(self.tau, 'shape'):
                ...
                tau = self.tau * torch.ones(state.shape[0], dtype=state.dtype, device=state.device)
